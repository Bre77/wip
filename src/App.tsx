import { useState, useCallback } from 'react';
import { useAuth } from './hooks/useAuth';
import { useItems } from './hooks/useItems';
import { Header } from './components/Header';
import { ItemList } from './components/ItemList';
import { RepoFilters } from './components/RepoFilters';
import { login } from './lib/api';

function App() {
  const { user, loading: authLoading } = useAuth();
  const authenticated = user?.authenticated ?? false;

  const {
    items,
    loading: itemsLoading,
    error,
    refreshing,
    refresh,
    changePriority,
    updateNotes,
    toggleHidden,
  } = useItems(authenticated);

  const [filteredRepos, setFilteredRepos] = useState<Set<string>>(new Set());

  const toggleRepo = useCallback((repo: string) => {
    setFilteredRepos(prev => {
      const next = new Set(prev);
      if (next.has(repo)) {
        next.delete(repo);
      } else {
        next.add(repo);
      }
      return next;
    });
  }, []);

  // Show loading state
  if (authLoading) {
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-gray-400">Loading...</div>
      </div>
    );
  }

  // Show login screen for unauthenticated users
  if (!authenticated) {
    return (
      <div className="min-h-screen bg-gray-900 flex flex-col">
        <Header user={user} refreshing={false} onRefresh={() => {}} />
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-white mb-4">
              Track Your GitHub Work in Progress
            </h2>
            <p className="text-gray-400 mb-8 max-w-md">
              View all your open PRs and issues, organized by priority with CI/CD status, merge conflicts, and review state at a glance.
            </p>
            <button
              onClick={() => login()}
              className="flex items-center gap-3 px-6 py-3 text-lg bg-[#24292f] hover:bg-[#32383f] text-white rounded-lg transition-colors mx-auto"
            >
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.604-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.167 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
              </svg>
              Sign in with GitHub
            </button>
          </div>
        </div>
      </div>
    );
  }

  const visibleCount = items.filter(i => !i.hidden).length;

  return (
    <div className="min-h-screen bg-gray-900 flex flex-col">
      <Header user={user} refreshing={refreshing} onRefresh={refresh} />

      {error && (
        <div className="px-6 py-3 bg-red-900/50 border-b border-red-800 text-red-200 text-sm">
          {error}
        </div>
      )}

      {itemsLoading ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-gray-400">Loading items...</div>
        </div>
      ) : (
        <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full">
          <div className="px-4 pt-3 flex items-center justify-between">
            <span className="text-sm text-gray-500">{visibleCount} items</span>
          </div>
          <RepoFilters
            items={items}
            selectedRepos={filteredRepos}
            onToggleRepo={toggleRepo}
          />
          <ItemList
            items={items}
            filteredRepos={filteredRepos}
            onChangePriority={changePriority}
            onUpdateNotes={updateNotes}
            onToggleHidden={toggleHidden}
          />
        </div>
      )}
    </div>
  );
}

export default App;
